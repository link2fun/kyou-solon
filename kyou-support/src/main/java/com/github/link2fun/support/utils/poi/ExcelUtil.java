package com.github.link2fun.support.utils.poi;
...
import com.github.link2fun.support.exception.UtilException;
...
import com.github.link2fun.support.utils.file.FileTypeUtils;
import com.github.link2fun.support.utils.file.FileUtils;
import com.github.link2fun.support.utils.file.ImageUtils;
...
public class ExcelUtil<T> {
  ...
  public void exportExcel(Context context, List<T> list, String sheetName) {
    exportExcel(context, list, sheetName, StringUtils.EMPTY);
  }
   ...
   * @param title     标题
   */
  public void exportExcel(final Context context, List<T> list, String sheetName, String title) {
   ...
   * @param sheetName 工作表的名称
   */
  public void importTemplateExcel(final Context context, String sheetName) {
    importTemplateExcel(context, sheetName, StringUtils.EMPTY);
  }
   ...
   * @param title     标题
   */
  public void importTemplateExcel(final Context context, String sheetName, String title) {
    context.contentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    context.charset("utf-8");
    this.init(null, sheetName, title, Excel.Type.IMPORT);
    exportExcel(context);
   ...
   * 对list数据源将其里面的数据导入到excel表单
   */
  public void exportExcel(final Context context) {
    try {
      writeSheet();
      ...
      final DownloadedFile downloadedFile = new DownloadedFile("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", inputStream, this.sheetName + ".xlsx");
  ...
  public void setCellVo(Object value, Excel attr, Cell cell) {
    if (Excel.ColumnType.STRING == attr.cellType()) {
      ...
      if (StringUtils.startWithAny(cellValue, FORMULA_STR)) {
      ...
      if (StringUtils.isNotEmpty(imagePath)) {
        ...
        byte[] data = ImageUtils.readBytes(imagePath);
  ...
  public void setDataValidation(Excel attr, int column) {
    ...
    if (StringUtils.isNotEmpty(attr.prompt()) || attr.combo().length > 0) {
      if (attr.combo().length > 15 || StrUtil.join("", (Object[]) attr.combo()).length() > 255) {
  ...
  public String dataFormatHandlerAdapter(Object value, Excel excel, Cell cell) {
    try {
      ...
      log.error("不能格式化数据{} {}", e.getMessage(), excel.handler());
    }
  ...
  public String getAbsoluteFile(String filename) {
    String downloadPath = Solon.context().getBean(KyouProperties.class).getDownloadPath() + filename;
