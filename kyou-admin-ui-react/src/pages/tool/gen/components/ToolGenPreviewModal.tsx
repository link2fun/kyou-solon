...
const ToolGenPreviewModal = memo(
  ({ open, onCancel, items = [], onRefresh }: ToolGenPreviewModalProps) => {
    ...
    const getMode = (filename: string): string => {
      ...
      if (filename.endsWith('js.vm') || filename.endsWith('vue.vm'))
        return 'javascript';
    ...
    return (
      ...
      >
        ...
        <span ref={ref}>
          <Highlight
            className={`${getMode(getActiveKey() || '')} h-96`}
            key={activeKey}
          >
            {activeContent}
          </Highlight>
